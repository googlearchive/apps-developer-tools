...
cr.define('extensions', function() {
   ...
   * @param {Object} chromeFunctions An object containing the functions to
   *     interact with the chrome browser. Includes entries for
   *     'requestFileSource' and 'openDevTools'.
   * @constructor
   * @extends {HTMLDivElement}
   */
  function ExtensionError(error, templateName, chromeFunctions) {
    var div = cloneTemplate(templateName);
    div.__proto__ = ExtensionError.prototype;
    div.error_ = error;
    div.chromeFunctions_ = chromeFunctions;
    div.decorate();
    return div;
  }
  ...
  ExtensionError.prototype = {
    ...
    createViewSourceLink_: function(url, line) {
      ...
      viewSource.addEventListener('click', function(e) {
        this.chromeFunctions_.requestFileSource(requestFileSourceArgs);
      }.bind(this));
    ...
    createInspectLink_: function(url, line, column) {
      ...
      inspectIcon.addEventListener('click', function(e) {
          this.chromeFunctions_.openDevTools(
   ...
   * @param {Object} chromeFunctions An object containing the functions to
   *     interact with the chrome browser. Includes entries for
   *     'requestFileSource' and 'openDevTools'. We need these so that we
   *     can use this interface both on chrome:extensions and in the
   *     Apps Developer Tool, which interact with the browser through
   *     different mechanisms.
   * @constructor
   * @extends {HTMLDivElement}
   */
  function ExtensionErrorList(errors, title, chromeFunctions) {
